...
int n; sscanf(rcvmsg, "%d", &n);
int status = n % 2 != 0;
int sq = ceil(sqrt(n) + 1);
for (int i = 3; status && i <= sq; i += 2) {
status = status & (n % i != 0);
}
if (status) sprintf(sendmsg, "Is Prime\n");
else sprintf(sendmsg, "Is not Prime\n");
}
...
int dp[n + 1], ndp[n + 1];
memset(dp, 0, (n + 1) * sizeof(int));memset(ndp, 0, (n + 1) * sizeof(int));
for (int i = 1; i <= n; i++) {
for (int j = 1; j <= n; j++) {
if (s1[i - 1] == s2[j - 1]) ndp[j] = 1 + dp[j - 1];
else ndp[j] = dp[j] > ndp[j - 1] ? dp[j] : ndp[j - 1];
}
...
int n = strlen(rcvmsg);
char rev[n];
for (int i = 0; i < n; i++) rev[i] = rcvmsg[n - i - 1];
sprintf(sendmsg, "%d\n", LCS(rcvmsg, rev, n));
}
...
printf("Enter the server port:"); scanf("%d", &servPort);
sd = socket(AF_INET, SOCK_STREAM, 0);
if (sd < 0) puts("Creation failed");
else puts("Socket Created");
servAddr.sin_family = AF_INET;
servAddr.sin_port = htons(servPort);
servAddr.sin_addr.s_addr = htonl(INADDR_ANY);
socketBind = bind(sd, (struct sockaddr*) &servAddr, sizeof(servAddr));
if (socketBind < 0) puts("Can't bind");
else puts("Binded");
listen(sd, 5);
cliLen = sizeof(cliAddr);
while (1) {
nsd = accept(sd, (struct sockaddr*) &cliAddr, &cliLen);
if (nsd < 0) puts("Can't accept");
else {
pid_t pid = fork();
if (pid == 0) {
...
void (*func)(char*, char*);if (choice == 1) func = &primes;
else if (choice == 2) func = &parenthesis;
else func = &LPS;
while (1) {
recv(nsd, rcvmsg, 101, 0);
printf("%s\n", rcvmsg);
